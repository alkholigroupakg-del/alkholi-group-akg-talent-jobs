  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <FormField label="هل ملتحق حالياً بدراسة أخرى؟" name="currentlyStudying" type="select" value={data.currentlyStudying || ""} onChange={onChange} options={["نعم", "لا"]} />
        {data.currentlyStudying === "نعم" && (
          <FormField label="ماهي الدراسة الحالية؟" name="currentStudy" type="text" value={data.currentStudy || ""} onChange={onChange} placeholder="وصف الدراسة الحالية" />
        )}
      </div>
    </div>
  );
